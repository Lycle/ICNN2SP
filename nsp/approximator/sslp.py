...
class ServerLocationProblemApproximator(Approximator):
    ...
    def get_master_mip(self):
        ...
        mip = gp.Model('mipQ')
        x_in = mip.addVars(self.inst['n_locations'], vtype=gp.GRB.BINARY, name='x_in')
        mip.update()
        objective = 0
        for i in x_in.keys():
            objective += self.inst['first_stage_costs'][i] * x_in[i]
    ...
    def approximate(self, n_scenarios, gap=0.02, time_limit=600, threads=1, log_dir=None, test_set="0", presolve=0, heuristics=0, nodemethod=1, cuts=1, mipfocus=2):
        ...
        approximator_mip.setParam('Presolve', presolve)
        approximator_mip.setParam('Heuristics', heuristics)
        approximator_mip.setParam('NodeMethod', nodemethod)
        approximator_mip.setParam('Cuts', cuts)
        approximator_mip.setParam('MIPFocus', mipfocus)
        # optimize
